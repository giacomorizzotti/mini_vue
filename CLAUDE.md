...
- **Display**: `TruncatedText` (show more/less for long text; opt-in `linkify` prop — see `useLinkify` below), `RichText` (auto-linked/line-broken plain text, no truncation — the non-truncating sibling of `TruncatedText`'s `linkify` mode, sharing the same `linkify()` function so both render identically), `MessageBox`, `LoadingBox`, `Loader`, `StarsBackground`, `Credits`, `Actions`, `Collapsible`
